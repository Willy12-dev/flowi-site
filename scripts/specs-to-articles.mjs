...
function buildArticle(spec) {
  ...
  const frontmatter = [
    ...
    // Hero image — the per-vertical fallback that already exists in
    // public/images/blog/, so the hero never 404s. (A per-slug hero
    // generator exists in scripts/generate-blog-heroes.mjs but is opt-in
    // and needs HIGGSFIELD_API_KEY; until that runs, use this fallback.)
    `image: "${IMAGE_BY_VERTICAL[vertical] || IMAGE_BY_VERTICAL.ai_general}"`,
    "---",
    "",
  ].join("\n");
